...
impl RankSelectIndex {
    ...
    pub fn rank(r: &Self, x: u8) -> u8 {
        ...
        let rank_table: [u8; 8] = [0b000000, 0b000001, 0b000101, 0b000110, 0b010101, 0b010110,
                                   0b011010, 0b011011];
    ...
    pub fn select(s: &Self, x: u8) -> u8 {
        ...
        // 2段目
        let c2 = 1;
        let (acc, second_idx) = if x <= s.tree[c2] {
            (0, c2)
        ...
        let (acc, third_idx) = if x <= acc + s.tree[c3] {
            (acc, c3)
        ...
        let (acc, forth_idx) = if x <= acc + s.tree[c4] {
            (acc, c4)
        ...
        let fifth_index = if x == acc + bits[0] {
            0
    ...
    fn create_select_index(b: u64) -> [u8; 40] {
        ...
        // 4段目
        for i in 0..21 {
            // tree[13 + i] : b[i*3]...b[i*3+2] のなかの1の個数
