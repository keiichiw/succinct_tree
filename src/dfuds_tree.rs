...
impl DFUDSTree {
    pub fn new(t: &Tree) -> DFUDSTree {
        let p = util::left_align(DFUDSTree::tree_to_bp(t.clone(), 0b1));
        let inv = RankSelectIndex::new(p ^ 0xffffffffffffffff);
        DFUDSTree{dfuds: BP::new(p), inv: inv }
    }

    pub fn ith_node(t: &DFUDSTree, i: u8) -> u8 {
        RankSelectIndex::select(&t.inv, i - 1)
    }

    pub fn node_id(t: &DFUDSTree, v: u8) -> u8 {
        RankSelectIndex::rank(&t.inv, v - 1) + 1
    }

    pub fn degree(t: &DFUDSTree, v: u8) -> u8 {
        let r = RankSelectIndex::rank(&t.inv, v);
        let s = RankSelectIndex::select(&t.inv, r + 1);
        s - v
    }

    pub fn ith_child(t: &DFUDSTree, v: u8, i: u8) -> u8 {
        let r = RankSelectIndex::rank(&t.inv, v);
        let s = RankSelectIndex::select(&t.inv, r + 1);
        let vj = BP::find_close(&t.dfuds, s-i) + 1;
        DFUDSTree::node_id(&t, vj) - 1
    }

    pub fn print(t: &DFUDSTree) {
        BP::print(&t.dfuds);
    }
        ...
        match t {
            ...
            Tree::Node(_, children) =>
            {
                ...
                for child in children {
                    bits = DFUDSTree::tree_to_bp(child, bits);
                };
                bits
            }
        }
